...
temp <- list.files(
  "../../data/mp-game-6/logTest/ThreeBeakers",
  pattern="*.csv",
  full.names=TRUE
)
...
three_beaker_graph <- accuracy_by_game %>%
  ...
  theme_cowplot() + 
  labs(title = "Student vs Teacher Accuracy", subtitle = "First Iteration (n = 5)", x = "Student Accuracy", y = "Teacher Accuracy", color = "Round") +
...
```{r}
  ...
  geom_point(data = accuracy_by_game_multi, mapping = aes(color = "#000099", x = student_accuracy, y = explorer_accuracy, shape = factor(round_num)), position = position_jitter(width = .01, height = .01)) +
  geom_smooth(data = accuracy_by_game, method = "lm", aes(color = "#CC0000", x = student_accuracy, y = explorer_accuracy)) +
  geom_smooth(data = accuracy_by_game_multi, method = "lm", aes(color = "#000099", x = student_accuracy, y = explorer_accuracy)) +
...
accuracy_by_question_type <- three_beaker_games %>%
  ...
  summarise(mean_accuracy = mean(is_correct), ci_low = (mean_accuracy - 1.96*sd(is_correct)/sqrt(n())), ci_up = (mean_accuracy + 1.96*sd(is_correct)/sqrt(n())), chance_accuracy = mean((str_count(true_answers, ",") + 1)/8)) 
...
accuracy_by_question_type %>%
  ...
  geom_col(aes(x = round_num, y = chance_accuracy, color = fixed), fill = "purple", alpha = .15, position = "dodge") +
...
```
```{r}
accuracy_by_question_type_multi <- multi_beaker_games %>%
  group_by(round_num, fixed) %>%
  summarise(mean_accuracy = mean(is_correct), ci_low = (mean_accuracy - 1.96*sd(is_correct)/sqrt(n())), ci_up = (mean_accuracy + 1.96*sd(is_correct)/sqrt(n())), chance_accuracy = mean((str_count(true_answers, ",") + 1)/8)) 

accuracy_by_question_type_multi
...
accuracy_by_question_type_multi %>%
  ...
  guides(fill = FALSE) + 
  theme_cowplot() + 
labs(title = "Accuracies across Question Type", subtitle = "Second Iteration (n = 7)", x = "Round", y = "Mean Accuracy", alpha = "Question Type", color = "Chance Accuracy")
...
accuracy_by_round_multi <- multi_beaker_games %>%
  group_by(round_num, role) %>%
  summarise(mean_accuracy = mean(is_correct), ci_low = (mean_accuracy - 1.96*sd(is_correct)/sqrt(n())), ci_up = (mean_accuracy + 1.96*sd(is_correct)/sqrt(n()))) 

accuracy_by_round_multi %>%
  ggplot(aes(x = round_num, y = mean_accuracy,)) + 
  geom_col(aes(fill = factor(round_num), alpha = role), position = "dodge") + 
  geom_errorbar(aes(ymin=ci_low, ymax=ci_up, alpha = role,),  color="black", width=0.5, position= position_dodge(.9), show.legend = FALSE) +
  scale_alpha_manual(values = c("explorer" = 1.0, "student" = 0.4), labels = c("explorer" = "Teacher", "student" = "Student")) +
  guides(fill = FALSE) + 
  theme_cowplot() + 
  labs(title = "Teacher and Student Accuracies across rounds", subtitle = "Second Iteration (n = 7)", x = "Round", y = "Mean Accuracy", alpha = "Role")
  
```

```{r}
all_games <- rbind(three_beaker_games, multi_beaker_games)

accuracy_by_question_multi <- all_games %>%
...
```{r}
accuracy_by_question <- all_games %>%
  group_by(round_num, fixed, config, role) %>%
...
all_accuracies_by_game <- rbind(accuracy_by_game, accuracy_by_game_multi)
...
game_times_and_accuracies %>%
  ...
  geom_smooth(method = "lm") + 
  theme_cowplot() + 
  labs(title = "Chat Time vs Teacher Accuracy", subtitle = "Both Iterations (n = 12)", color = "Round", y = "Teacher Accuracy", 
       x = "Chat Time (s)") 
...
game_times_split_all %>%
  ...
  theme_cowplot() + 
  labs(title = "Time spent learning vs teaching", subtitle = "Both Iterations (n = 12)", x = "Language Type", y = "Mean Time (s)", fill = "Language Type", alpha = "Task") +
  guides(fill = FALSE)
...
```{r}
accuracy_by_question_by_role <- all_games %>%
  group_by(round_num, fixed, config, role) %>%
  summarise(mean_accuracy = mean(is_correct)) %>%
  mutate(role = paste0(role, "_accuracy")) %>%
  spread(role, mean_accuracy)
...
accuracy_by_question_by_role %>%
  ...
  theme_cowplot() + 
  labs(title = "Student vs Teacher Accuracy by Question", subtitle = "Both Iterations (n = 12)", color = "Question Type", x = "Student Accuracy", y = "Teacher Accuracy") +
