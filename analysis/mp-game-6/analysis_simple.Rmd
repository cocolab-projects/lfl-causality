...
three_beaker_graph <- accuracy_by_game %>%
  ...
  geom_point(aes(color = factor(round_num)), position = position_jitter(width = .01, height = .01)) +
  geom_smooth(method = "lm") +
  theme_cowplot() + 
  labs(title = "Student vs Teacher Accuracy", subtitle = "First Iteration (n = 5) p = .0228", x = "Student Accuracy", y = "Teacher Accuracy", color = "Round") +
...
game_times_and_accuracies %>%
  ...
  geom_smooth(method = "lm") + 
  theme_cowplot() + 
  labs(title = "Chat Time vs Explorer Accuracy", subtitle = "Both Iterations (n = 5) p = .38", color = "Round", y = "Teacher Accuracy", 
       x = "Chat Time (s)") 
...
game_times_split_brute <- game_times_brute %>%
  ...
  group_by(Type) %>%
  summarise(average_time = mean(Time), ci_low = (average_time - 1.96*sd(Time)/sqrt(n())), ci_up = (average_time + 1.96*sd(Time)/sqrt(n())), lang = "Brute")

game_times_split_causal <- game_times_causal %>%
  gather(Type, Time, c(`Total Train Time`, `Total Test Time`, chat)) %>%
  group_by(Type) %>%
  summarise(average_time = mean(Time), ci_low = (average_time - 1.96*sd(Time)/sqrt(n())), ci_up = (average_time + 1.96*sd(Time)/sqrt(n())), lang = "Causal")
...
game_times_split_all %>%
  ...
  theme_cowplot() + 
  labs(title = "Time spent learning vs teaching", subtitle = "First Iterations (n = 5)", x = "Language Type", y = "Mean Time (s)", fill = "Language Type", alpha = "Task") +
  guides(fill = FALSE)
...
accuracy_by_question_by_role %>%
  ...
  theme_cowplot() + 
  labs(title = "Student vs Teacher Accuracy by Question", subtitle = "Both Iterations (n = 5) p = .0054", color = "Question Type", x = "Student Accuracy", y = "Teacher Accuracy") +
...
agreement %>%
  ...
  theme_cowplot() + 
  geom_abline(intercept = 0, slope = 1, linetype = "dotted") + 
  labs(title = "Student vs Teacher Question Accuracy", subtitle = "(n = 5), p = 4.491e-09", x = "Student Accuracy", y = "Teacher Accuracy", color = "Round") + 
  xlim(-.05, 1.2) + 
  ylim(-.05, 1.2)
...
agreement_by_round_game %>%
  ...
  theme_cowplot() + 
  geom_abline(intercept = 0, slope = 1, linetype = "dotted") + 
  labs(title = "Chance vs Actual Agreement", subtitle = "(n = 5), p = 1.51e-07", x = "Chance Agreement", y = "Actual Agreement", color = "Round") + 
  xlim(-.05, 1.05) + 
  ylim(-.05, 1.05)
