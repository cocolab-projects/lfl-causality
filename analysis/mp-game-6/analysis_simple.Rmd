...
cor_times <- cor.test(game_times_and_accuracies$chat, game_times_and_accuracies$explorer_accuracy)

cor_times

game_times_and_accuracies %>%
  ggplot(aes(x = chat, y = explorer_accuracy)) + 
  geom_point(aes(color = factor(round_num))) + 
  geom_smooth(method = "lm") + 
  theme_cowplot() + 
  labs(title = "Chat Time vs Explorer Accuracy", subtitle = "Both Iterations (n = 5) p = .93", color = "Round", y = "Teacher Accuracy", 
       x = "Chat Time (s)") 
